...
new_row <- data.frame(from = integer(1),
					 to = integer(1),
					 weight = numeric(1),
					 type = character(1),
					 link_width = numeric(1)
					)
...
#	print(ii)
	...
	if (length(enfant_indx))
	{
		...
		enfant <- gsub("@", "", enfant)
		enfant <- as.numeric(enfant)
		for (ienf in 1:length(enfant))
		{
#			cat(ii, cpl[ii,3], enfant[ienf], cpl[ii,3], "child\n")
	
			new_row$from[1] <- as.integer(cpl[ii,4])
			new_row$to[1] <- as.integer(enfant[ienf])
			new_row$weight[1] <- 1
			new_row$type[1] <- "child"
			new_row$link_width[1] <- 100
			child_edge <- rbind(child_edge, new_row)
		}
	}
